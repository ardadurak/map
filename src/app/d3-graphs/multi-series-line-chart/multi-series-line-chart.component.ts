...
export class MultiSeriesLineChartComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    d3G = d3Svg
      ...
      y.domain([
        d3.min(processedStocks, function(c) { return d3.min(c.values, function(d) { return d.change; }); }),
        d3.max(processedStocks, function(c) { return d3.max(c.values, function(d) { return d.change; }); })
      ]);
